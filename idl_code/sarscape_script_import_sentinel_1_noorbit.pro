pro  SARscape_script_import_Sentinel_1

  compile_opt idl2

  infoRoutine = ROUTINE_INFO('SARscape_script_import_Sentinel_1', /SOURCE)
  ...
  ; inOrbits = CLI_args[1] + PATH_SEP()+ CLI_args[2]
  
  
  ; 1) SARscape batch initialization and temporary directory setting
  if (strpos(!prompt,'ENVI') eq -1 ) then begin
    aTmp=inImages+PATH_SEP()+params['import_dir']+PATH_SEP()+ params['import_work_dir']
    FILE_MKDIR,aTmp
